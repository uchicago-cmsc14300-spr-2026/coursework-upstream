...
// ======== new operations for HW6

// return the legal moves for a given knight
// precondition: there is a knight at the location
// precondition: the knight's color matches whose turn
// ERROR if the location is unoccupied
// ERROR if the location is occupied by a non-knight
// ERROR if the knight's color does not match whose turn
// moves can appear in any order in the result
move_list *knight_moves(board b, loc_t loc);

// return the legal moves for a given envoy
// precondition: there is an envoy at the location
// precondition: the envoy's color matches whose turn
// must exclude the shadow realm (see rules)
// ERROR if the location is unoccupied
// ERROR if the location is occupied by a non-envoy
// ERROR if the envoy's color does not match whose turn
// moves can appear in any order in the result
move_list *envoy_moves(board b, loc_t loc);

// return the legal moves for the player whose turn it is
// moves can appear in any order in the result
move_list *possible_moves(board b);

// perform the specified move:
// - move piece from source to destination
// - perform any capture that is a consequence of the move
// - advance the turn
// return true if move is legal and has been completed
// - return false if move was illegal, and do nothing
int perform_move(board *b, move m);
